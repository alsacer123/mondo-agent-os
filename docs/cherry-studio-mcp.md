...
- 检查第一位内测执行包、触达清单和准入记录是否都已准备好，并返回下一步动作。
...
7. 准备第一位真实用户内测时，可以调用 mondo_prepare_beta_pack 生成本地执行包，调用 mondo_prepare_beta_outreach 生成候选触达清单，并调用 mondo_prepare_beta_intake 生成候选用户准入记录；再调用 mondo_beta_status 检查材料是否齐全，按 `docs/beta-candidate-outreach.md`、`docs/beta-user-selection.md` 和 `docs/beta-user-intake-template.md` 先筛选候选用户。
